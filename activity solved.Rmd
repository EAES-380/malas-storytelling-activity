...
landfill_df %>% 
  group_by(sample) %>%
  mutate(archaeal_abundance = sum(abundance[kingdom == "Archaea"])) %>%
  ungroup() %>%
  mutate(sample = fct_reorder(sample, archaeal_abundance)) %>%
  ggplot(aes(sample, abundance, color = kingdom, fill = kingdom))+
  geom_bar(stat="identity")+
  coord_flip()
...
landfill_df %>% 
  ...
  group_by(temp_grp, genus) %>% 
   #calculate the average relative abundance per genus within the temperature groups 
  summarize(mean_relative_abundance = mean(abundance)*100, .groups = "drop") %>% 
  filter(mean_relative_abundance > 1) %>% 
  drop_na() %>% 
